// ObsidianSDK client (generated — mirrors the rust Main fragment).
...
ObsidianSDK* obsidian_sdk_new(voxgig_value* options) {
  ObsidianSDK* sdk = (ObsidianSDK*)calloc(1, sizeof(ObsidianSDK));
...
static bool sdk_op_allowed(ObsidianSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(ObsidianSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
  const char* allow = voxgig_is_string(allow_op) ? voxgig_as_string(allow_op) : "";
  char msg[512];
  snprintf(msg, sizeof(msg),
    "ObsidianSDK: %s: operation not allowed by"
...
static voxgig_value* sdk_raw_request(
  ObsidianSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(ObsidianSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "ObsidianSDK: graphql: %s", m);
...
// Vault entity bound to this client.
Entity* obsidian_vault(ObsidianSDK* client, voxgig_value* entopts) {
  return vault_entity_new(client, entopts);
}
...
ObsidianSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  ObsidianSDK* sdk = obsidian_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
