// Generated direct-call test for the command entity (mirrors the
...
static voxgig_value* command_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static ObsidianSDK* command_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(command_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    ObsidianSDK* sdk = command_direct_setup(mockres);
  ...
  TEST_SUMMARY("obsidian_command_direct");
}
