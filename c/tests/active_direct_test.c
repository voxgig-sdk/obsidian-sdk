// Generated direct-call test for the active entity (mirrors the
...
static voxgig_value* active_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static ObsidianSDK* active_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(active_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    ObsidianSDK* sdk = active_direct_setup(mockres);
  ...
  TEST_SUMMARY("obsidian_active_direct");
}
