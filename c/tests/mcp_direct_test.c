// Generated direct-call test for the mcp entity (mirrors the
...
static voxgig_value* mcp_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static ObsidianSDK* mcp_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(mcp_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    ObsidianSDK* sdk = mcp_direct_setup(mockres);
  ...
  TEST_SUMMARY("obsidian_mcp_direct");
}
