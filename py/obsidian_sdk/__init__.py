...
from obsidian_sdk.utility import register

# Load features
from obsidian_sdk.feature.base_feature import ObsidianBaseFeature
from obsidian_sdk.features import _has_feature, _make_feature
...
class ObsidianSDK:
    ...
    def get_utility(self):
        return ObsidianUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("ObsidianSDK: graphql: " + str(msg))
            res["graphql"] = errors
    ...
    def Vault(self, data=None) -> "VaultEntity":
        """Entity factory: client.Vault().list() / client.Vault().load({"id": ...})."""
        from obsidian_sdk.entity.vault_entity import VaultEntity
        return VaultEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "ObsidianSDK":
        if sdkopts is None:
            sdkopts = {}
