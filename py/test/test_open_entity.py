...
class TestOpenEntity:

    def test_should_create_instance(self):
        testsdk = ObsidianSDK.test(None, None)
        ent = testsdk.Open(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _open_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "open." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OBSIDIAN_TEST_OPEN_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        open_ref01_ent = client.Open(None)
        ...
        open_ref01_data = helpers.to_map(runner.entity_data(open_ref01_ent.create(open_ref01_data, None)))
        assert open_ref01_data is not None
        assert open_ref01_data["id"] is not None



def _open_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/open/OpenTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("OBSIDIAN_TEST_OPEN_ENTID"))
