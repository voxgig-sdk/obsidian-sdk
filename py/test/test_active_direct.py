...
class TestActiveDirect:

    def test_should_direct_load_active(self):
        setup = _active_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-active", "live" if setup["live"] else "unit")
        if _skip:
...
def _active_direct_setup(mockres):
