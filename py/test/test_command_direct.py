...
class TestCommandDirect:

    def test_should_direct_list_command(self):
        setup = _command_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-command", "live" if setup["live"] else "unit")
        if _skip:
...
def _command_direct_setup(mockres):
