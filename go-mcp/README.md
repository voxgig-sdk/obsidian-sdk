...
[MCP](https://modelcontextprotocol.io) server exposing the Obsidian SDK as
two agent tools — `obsidian_list` and `obsidian_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/obsidian-mcp)
make build
...
claude mcp add --scope user obsidian \
  -- /absolute/path/to/obsidian-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./obsidian-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// obsidian_list: first page of records
{ "entity": "command" }
{ "entity": "command", "query": { } }

// obsidian_load: one record by id
{ "entity": "active", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/obsidian-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user obsidian \
     -- "$PWD"/dist/*/obsidian-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `obsidian_list` and `obsidian_load` tools now appear
   in new sessions. Ask the agent to *"list command using obsidian"*
   and it calls `obsidian_list` with `{"entity":"command"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `obsidian_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `obsidian_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OBSIDIAN_APIKEY` | API key sent with every request. |
| `OBSIDIAN_BASE` | Optional override of the API base URL. |
...
```sh
./obsidian-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"obsidian_load","arguments":{"entity":"active","query":{"id":1}}}}'
```
