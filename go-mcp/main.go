// Command Obsidian-mcp is an MCP server exposing the Obsidian
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Obsidian_list  — list records of an entity
//	Obsidian_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: OBSIDIAN_APIKEY carries the API key and
	// OBSIDIAN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OBSIDIAN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OBSIDIAN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewObsidianSDK(opts)
