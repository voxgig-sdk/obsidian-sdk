...
func (sdk *ObsidianSDK) OptionsMap() map[string]any {
...
func (sdk *ObsidianSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *ObsidianSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *ObsidianSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ObsidianSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *ObsidianSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *ObsidianSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("ObsidianSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *ObsidianSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ObsidianSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("ObsidianSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func (sdk *ObsidianSDK) Tag(data map[string]any) ObsidianEntity {
	return NewTagEntityFunc(sdk, data)
}


// Vault returns a Vault entity bound to this client.
// Idiomatic usage: client.Vault(nil).List(nil, nil) or
// client.Vault(nil).Load(map[string]any{"id": ...}, nil).
func (sdk *ObsidianSDK) Vault(data map[string]any) ObsidianEntity {
	return NewVaultEntityFunc(sdk, data)
}
