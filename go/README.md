...
It exposes the API as capitalised, semantic **Entities** — e.g. `client.Tag(nil)` — each with the same small set of operations (`List`, `Load`, `Create`, `Update`, `Remove`, `Patch`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
func main() {
    ...
    // List tag records — the value is the array of records itself.
    tags, err := client.Tag(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range tags.([]any) {
        fmt.Println(item)
    }
}
```
...
tag, err := client.Tag(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(tag) // the returned mock data
```
...
slice):
    ...
    tag, err := client.Tag(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // tag is the returned record
