...
func TestMcpEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OBSIDIAN_TEST_MCP_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if mcpRef01Data["id"] == nil {
			t.Fatal("expected created entity to have an id")
		}

		// LOAD
		mcpRef01MatchDt0 := map[string]any{
			"id": mcpRef01Data["id"],
		}
		mcpRef01DataDt0Loaded, err := mcpRef01Ent.Load(mcpRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		mcpRef01DataDt0LoadResult := core.ToMapAny(entityData(mcpRef01DataDt0Loaded))
		if mcpRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if mcpRef01DataDt0LoadResult["id"] != mcpRef01Data["id"] {
...
func mcpBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OBSIDIAN_TEST_MCP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OBSIDIAN_TEST_MCP_ENTID"])
