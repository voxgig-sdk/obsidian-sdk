...
raise a `ObsidianError` (a `StandardError` subclass), so wrap
...
```ruby
# load returns the ENTITY — call data_get for the Vault record (raises on error).
...
```ruby
# list returns an Array of Vault records (raises on error).
