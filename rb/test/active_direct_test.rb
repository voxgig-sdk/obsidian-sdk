...
class ActiveDirectTest < Minitest::Test
  def test_direct_load_active
    setup = active_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-active", setup[:live] ? "live" : "unit")
    if _should_skip
...
def active_direct_setup(mockres)
