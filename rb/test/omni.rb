...
module ObsidianOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if ObsidianOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = ObsidianOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| ObsidianOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || ObsidianOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = ObsidianSpec.new(spec_map) if spec_map.is_a?(::Hash)

    res_map = ctxmap['result']
    if res_map.is_a?(::Hash)
      ctx.result = ObsidianResult.new(res_map)
      err_map = res_map['err']
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
         !err_map['message'].empty?
        ctx.result.err = ObsidianError.new('', err_map['message'])
      end
    end

    resp_map = ctxmap['response']
    if resp_map.is_a?(::Hash)
      ctx.response = ObsidianResponse.new(resp_map)
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { ObsidianOmni.errify(err) }
