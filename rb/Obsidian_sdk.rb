...
require_relative 'Obsidian_types'
...
class ObsidianSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ObsidianHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ObsidianHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ObsidianFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ObsidianHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ObsidianHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ObsidianHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ObsidianHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ObsidianError.new(
        "graphql_error", "ObsidianSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Vault.list / client.Vault.load({ "id" => ... })
  def Vault(data = nil)
    require_relative 'entity/vault_entity'
    VaultEntity.new(self, data)
  end
