...
module ObsidianUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ObsidianContext.new({}, nil)
    end
    op = ctx.op || ObsidianOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ObsidianResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ObsidianError) ? err.msg : err.to_s
    ...
    sdk_err = ObsidianError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ObsidianError)
