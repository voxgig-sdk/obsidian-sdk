...
module ObsidianUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = ObsidianResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ObsidianResponse.new(fetched)
    else
