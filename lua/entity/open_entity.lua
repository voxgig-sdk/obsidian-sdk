...
local OpenEntity = {}
OpenEntity.__index = OpenEntity


function OpenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OpenEntity)
  self._name = "open"
  self._client = client
...
function OpenEntity:get_name()
  return self._name
end


function OpenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OpenEntity.new(self._client, opts)
end
...
function OpenEntity:mark_deleted()
  self._deleted = true
end


function OpenEntity:deleted()
  return true == self._deleted
end


function OpenEntity:data_set(args)
  if args ~= nil then
...
function OpenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OpenEntity:match_set(args)
  if args ~= nil then
...
function OpenEntity:stream(action, args, callopts)
...
---@param reqdata OpenCreateData
---@param ctrl? table
---@return Open
---@return string? err
function OpenEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function OpenEntity:_run_op(ctx, post_done)
  local utility = self._utility
