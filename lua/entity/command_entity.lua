...
local CommandEntity = {}
CommandEntity.__index = CommandEntity


function CommandEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CommandEntity)
  self._name = "command"
  self._client = client
...
function CommandEntity:get_name()
  return self._name
end


function CommandEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CommandEntity.new(self._client, opts)
end
...
function CommandEntity:mark_deleted()
  self._deleted = true
end


function CommandEntity:deleted()
  return true == self._deleted
end


function CommandEntity:data_set(args)
  if args ~= nil then
...
function CommandEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CommandEntity:match_set(args)
  if args ~= nil then
...
function CommandEntity:match_get()
...
function CommandEntity:stream(action, args, callopts)
...
---@param reqmatch CommandListMatch
---@param ctrl? table
---@return Command[]
---@return string? err
function CommandEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CommandCreateData
---@param ctrl? table
---@return Command
---@return string? err
function CommandEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CommandEntity:_run_op(ctx, post_done)
  local utility = self._utility
