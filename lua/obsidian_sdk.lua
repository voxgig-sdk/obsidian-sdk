...
local ObsidianSDK = {}
ObsidianSDK.__index = ObsidianSDK
...
ObsidianSDK._make_feature = _make_feature


function ObsidianSDK.new(options)
  local self = setmetatable({}, ObsidianSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ObsidianSDK:options_map()
...
function ObsidianSDK:get_utility()
  return Utility.copy(self._utility)
end


function ObsidianSDK:get_root_ctx()
  return self._rootctx
end


function ObsidianSDK:prepare(fetchargs)
...
function ObsidianSDK:direct(fetchargs)
...
-- by passing it.
function ObsidianSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ObsidianSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ObsidianSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:Vault():list() / client:Vault():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function ObsidianSDK:Vault(data)
  local EntityMod = require("entity.vault_entity")
  if data == nil then
    if self._vault == nil then
      self._vault = EntityMod.new(self, nil)
    end
    return self._vault
  end
  return EntityMod.new(self, data)
...
function ObsidianSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ObsidianSDK.new(sdkopts)
  sdk.mode = "test"
