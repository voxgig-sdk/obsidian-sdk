...
(0, node_test_1.describe)('CommandEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when OBSIDIAN_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('OBSIDIAN_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.ObsidianSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['create', 'list']) {
            if (!live && (0, utility_1.maybeSkipControl)(t, 'entityOp', 'command.' + op, live))
                return;
        }
        ...
        (0, node_assert_1.default)(null != command_ref01_data.id);
        // LIST
        const command_ref01_match = {};
        const command_ref01_list = (await command_ref01_ent.list(command_ref01_match)).map((e) => e.data());
        (0, node_assert_1.default)(!isempty(select(command_ref01_list, { id: command_ref01_data.id })));
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/command/CommandTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.ObsidianSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['command01', 'command02', 'command03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        ...
        client = new __1.ObsidianSDK(merge([
...
//# sourceMappingURL=CommandEntity.test.js.map