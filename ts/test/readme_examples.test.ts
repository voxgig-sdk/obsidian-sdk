...
function isRunnable(code: string): boolean {
  return (
    /new\s+ObsidianSDK\b/.test(code) ||
    /\bObsidianSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ObsidianSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ObsidianSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
