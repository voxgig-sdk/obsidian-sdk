...
{
    private static function utility(): ObsidianUtility
    {
        return new ObsidianUtility();
    }
    ...
    private static function ctx(array $over = []): ObsidianContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ObsidianContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ObsidianOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ObsidianError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ObsidianOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ObsidianMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ObsidianOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ObsidianMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ObsidianOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ObsidianMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ObsidianSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ObsidianMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ObsidianResponse([]),
            'result' => new ObsidianResult([]),
        ]);
        [, $err] = ObsidianMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ObsidianSpec([]),
            'response' => null,
            'result' => new ObsidianResult([]),
        ]);
        [, $err] = ObsidianMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ObsidianSpec(['step' => 's']),
            'response' => new ObsidianResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ObsidianResult([]),
        ]);
        [, $err] = ObsidianMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ObsidianSpec(['step' => 's']),
            'response' => new ObsidianResponse(self::resp(200, ['v' => 1])),
            'result' => new ObsidianResult([]),
        ]);
        [, $err] = ObsidianMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ObsidianSpec(['step' => 's']),
            'response' => new ObsidianResponse(self::resp(200, ['v' => 2])),
            'result' => new ObsidianResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ObsidianMakeResponse::call($ctx);
    ...
    {
        $preset = new ObsidianResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ObsidianSpec([]),
            'response' => new ObsidianResponse([]),
            'result' => new ObsidianResult([]),
        ...
        [$got, $err] = ObsidianMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ObsidianResult([])]);
        [, $err] = ObsidianMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ObsidianSpec([]), 'result' => null]);
        [, $err] = ObsidianMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ObsidianSpec(['step' => 's']),
            'result' => new ObsidianResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ObsidianOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ObsidianMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ObsidianSpec(['step' => 's']),
            'result' => new ObsidianResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ObsidianOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ObsidianMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ObsidianResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ObsidianSpec([]),
            'result' => new ObsidianResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ObsidianMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ObsidianMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ObsidianError('boom', 'boom');
        $utility->fetcher = function (ObsidianContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ObsidianSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ObsidianMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ObsidianContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ObsidianMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ObsidianResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ObsidianContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ObsidianSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ObsidianMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ObsidianResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ObsidianSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ObsidianMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ObsidianMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ObsidianMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ObsidianResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ObsidianDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ObsidianResult(['ok' => false])]);
        $this->expectException(ObsidianError::class);
        ObsidianDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ObsidianResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ObsidianMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ObsidianMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ObsidianBaseFeature();
        $b = new ObsidianBaseFeature();
        ObsidianFeatureAdd::call($ctx, $a);
        ObsidianFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ObsidianBaseFeature
    {
        $f = new ObsidianBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ObsidianFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ObsidianContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ObsidianSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ObsidianPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ObsidianResponse(['headers' => null]),
            'result' => new ObsidianResult([]),
        ]);
        ObsidianResultHeaders::call($ctx);
