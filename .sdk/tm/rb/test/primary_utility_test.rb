...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ObsidianOmni.make_runner(TEST_JSON_FILE, ObsidianSDK.test(nil, nil))
  ...
  def test_feature_add_basic
    ...
    feature = ObsidianBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = ObsidianSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = ObsidianSDK.test(nil, nil)
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = ObsidianSDK.new({
      "base" => "http://localhost:8080",
      "system" => {
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = ObsidianSpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = ObsidianSDK.test(nil, setup_opts)
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ObsidianOmni.errify(err)
    end
  ...
  def test_make_url_basic
    subject = lambda do |ctx|
      ctx.result = ObsidianResult.new({}) unless ctx.result
  ...
  def test_prepare_auth_basic
    ...
    auth_client = ObsidianSDK.test(nil, setup_opts)
