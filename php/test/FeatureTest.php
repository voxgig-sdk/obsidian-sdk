...
class FtCtrl extends ObsidianControl
{
...
class FtClient
{
    ...
    private ObsidianUtility $utility;

    public function __construct(ObsidianUtility $utility, array $options)
    {
    ...
    public function get_utility(): ObsidianUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ObsidianContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ObsidianContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ObsidianContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ObsidianConfig::make_feature($name);
    ...
    private static function buildUrl(ObsidianSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ObsidianError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ObsidianResponse($fetched);
        }
    ...
    private function fail(ObsidianContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ObsidianContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
