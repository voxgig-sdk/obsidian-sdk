...
class ObsidianMakeError
{
    public static function call(?ObsidianContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ObsidianContext([], null);
        }
        $op = $ctx->op ?? new ObsidianOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ObsidianResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ObsidianError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ObsidianError('', $msg, $ctx);
