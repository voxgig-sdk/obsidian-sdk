...
class ObsidianSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new ObsidianUtility();
        $this->_utility = $utility;

        $config = ObsidianConfig::shared_config();
        ...
        $feature_opts = ObsidianHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ObsidianHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ObsidianFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ObsidianFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ObsidianUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ObsidianHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ObsidianHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ObsidianHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ObsidianError($op . "_allow",
                "ObsidianSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ObsidianHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ObsidianError("graphql_error",
                "ObsidianSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Vault()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->vault()
    // resolves here too.
    public function Vault($data = null)
    {
        require_once __DIR__ . '/entity/vault_entity.php';
        if ($data === null) {
            if ($this->_vault === null) {
                $this->_vault = new VaultEntity($this, null);
            }
            return $this->_vault;
        }
        return new VaultEntity($this, $data);
    }
