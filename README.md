...
Obsidian Local REST API client, generated from the OpenAPI spec.

The Local REST API plugin for Obsidian, which serves the open vault over HTTP on the machine Obsidian is running on.

Learn more about Obsidian Local REST API at [127.0.0.1](http://127.0.0.1).

This is an unofficial SDK for the Obsidian Local REST public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Tag and Vault — that you
...
const items = await client.Tag().list()
```
...
// tags is an array of Tag entities, populated with mock data
// — call tags[0].data() for the record itself
console.log(tags)
```
...
```go
client := sdk.Test()
result, err := client.Tag(nil).List(
    nil, nil,
)
```
...
// List all tags (returns TagEntity[] — .data() for the record)
const tags = await client.Tag().list()
...
# List all tags (returns a list, raises on error)
tags = client.Tag().list()
for tag in tags:
    print(tag)
```
...
// List all tags (returns an array; throws on error)
$tags = $client->Tag()->list();
print_r(array_map(fn($item) => $item->data_get(), $tags));
```
...
# List all tags (returns an Array; raises on error)
tags = client.Tag.list
puts tags
```
...
// List all tags (returns a List, sets *err on failure)
voxgig_value* tags = tag->vt->list(tag, NULL, NULL, &err);
...
- Upstream API: [http://127.0.0.1:27123](http://127.0.0.1:27123)
...
Generated from the Obsidian Local REST API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
